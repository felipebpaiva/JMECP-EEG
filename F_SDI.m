%% SCRIPT 5: Structural Decoupling Index (SDI) Calculation
% This script implements the Graph Signal Processing method described by
% Preti & Van De Ville (Nature Communications, 2019) to calculate the
% Structural Decoupling Index (SDI) for each brain region.
%
% Adaptation: Uses each subject's global average functional connectome
% (ciPLV_global) as the "structure" and the z-scored alpha power time series
% as the "signal".

clear all;
clc;
addpath('K:\JMECP_EEG_Analysis\Scripts\EEGLAB\eeglab2025.0.0')
eeglab 

%% --- 1. Load Preprocessed Data ---
disp('Loading preprocessed data...');
load('K:\JMECP_EEG_Analysis\results\RELAXv2\Preprocessed_GED_Graphs_5s.mat');
disp('Data loaded.');
...
if size(alphaPower, 1) == ntrials && size(alphaPower, 2) == Nroi
    alphaPower_permuted = permute(alphaPower, [2, 3, 1]); % nodes x subjects x trials
else
    alphaPower_permuted = alphaPower;
end
data_Az = zscore(alphaPower_permuted, 0, 1); % Z-score across nodes

% Concatenate trials to get full time series: nodes x subjects x timepoints
if exist('EEG', 'var') && isfield(EEG, 'pnts') && ~isempty(EEG.pnts)
    points_per_trial = EEG.pnts;
else
    points_per_trial = size(alphaPower_permuted, 3);
    warning('EEG.pnts not found, estimating points per trial from alphaPower dimensions. Ensure this is correct.');
end
if numel(points_per_trial) > 1, points_per_trial = points_per_trial(1); end
total_timepoints = ntrials * points_per_trial;
alpha_ts_full = reshape(data_Az, Nroi, nsubj, []); % Reshape to nodes x subj x time

%% --- 3. Calculate Average Energy Spectrum & Median Split (Using Global Graphs) ---
disp('Calculating average energy spectrum from global graphs to find median split...');
avg_energy_spectrum = zeros(Nroi, 1);
num_valid_spectra = 0;
alpha_ts_full_reshaped = reshape(alphaPower_permuted, Nroi, nsubj, []);
...
for subi = 1:nsubj
    ...
    signal_subj = squeeze(alpha_ts_full_reshaped(:, subi, :));
...
%% --- 4. Calculate Subject-Specific, Trial-Averaged SDI using Dynamic Graphs ---
disp('Calculating subject-specific SDI maps using trial graphs...');
...
parfor subi = 1:nsubj
    ...
    sdi_log2_trials_subj = nan(Nroi, ntrials);
    ...
    for triali = 1:ntrials
        ...
        signal_trial = squeeze(data_Az(:, subi, triali));
        gft_coeffs_trial = V_trial' * signal_trial;

        gft_coeffs_coupled = gft_coeffs_trial;
        gft_coeffs_coupled(median_split_idx+1:end) = 0;
        signal_coupled = V_trial * gft_coeffs_coupled;

        gft_coeffs_decoupled = gft_coeffs_trial;
        gft_coeffs_decoupled(1:median_split_idx) = 0;
        signal_decoupled = V_trial * gft_coeffs_decoupled;

        energy_coupled = real(signal_coupled.^2);
        energy_decoupled = real(signal_decoupled.^2);
...
parfor i = 1:n_perms
    ...
    % **FIX**: Corrected conditional assignment syntax
    if ~isempty(max_t_val)
        max_t_stat_perm(i) = max_t_val;
    else
        max_t_stat_perm(i) = 0;
    end
end
...
for g = 1:length(groups_to_analyze)
    ...
    for chan = 1:Nroi
        ...
        if sum(valid_mask) > 2
            r_real(chan) = corr(sdi_group(chan, valid_mask)', wasi_group(valid_mask));
        end
    end
    ...
    parfor i = 1:n_perms
        ...
        for chan = 1:Nroi
            ...
            if sum(valid_mask) > 2
                r_perm(chan) = corr(sdi_group(chan, valid_mask)', perm_wasi(valid_mask));
            end
        end
        max_r_val = max(abs(r_perm), [], 'omitnan');
        % **FIX**: Corrected conditional assignment syntax
        if ~isempty(max_r_val)
             max_r_stat_perm(i) = max_r_val;
        else
             max_r_stat_perm(i) = 0;
        end
    end
    ...
    % Find threshold and significant electrodes
...
% Get the real correlation maps calculated in the previous step
r_controls = corr_results.Controls.r_real;
r_patients = corr_results.Patients.r_real;
diff_map_real = r_patients - r_controls; % Patient correlation - Control correlation

max_diff_stat_perm = nan(n_perms, 1);
disp('Running permutations for correlation differences...');
parfor i = 1:n_perms
    perm_labels = labels_all(randperm(length(labels_all))); % Shuffle group labels across everyone
    
    % Recalculate correlations within the permuted groups
    ...
    for chan = 1:Nroi
        ...
        if sum(valid_mask0) > 2
            r_perm_group0(chan) = corr(sdi_log2(chan, perm_mask0(valid_mask0))', wasi(perm_mask0(valid_mask0)));
        end
        
        % Permuted Group 1
        perm_mask1 = (perm_labels == 1);
        valid_mask1 = ~isnan(sdi_log2(chan, perm_mask1))' & ~isnan(wasi(perm_mask1));
        if sum(valid_mask1) > 2
             r_perm_group1(chan) = corr(sdi_log2(chan, perm_mask1(valid_mask1))', wasi(perm_mask1(valid_mask1)));
        end
    end
...
save('K:\JMECP_EEG_Analysis\results\RELAXv2\SDI_Results_DynamicGraphs.mat', ...
    ...
    'corr_results'); % Updated corr_results includes the difference analysis
...
fig_avg = figure('Name', 'Average SDI & Significant Differences (Dynamic Graphs - log2 ratio)');
t_avg = tiledlayout(2, 2, 'Padding', 'compact');

nexttile(t_avg);
topoplot(sdi_controls_avg, EEG.chanlocs, 'maplimits', common_maplimits_avg); colorbar; title('Controls Avg SDI (log2)');
nexttile(t_avg);
topoplot(sdi_patients_avg, EEG.chanlocs, 'maplimits', common_maplimits_avg); colorbar; title('Patients Avg SDI (log2)');
nexttile(t_avg);
topoplot(sdi_diff_avg, EEG.chanlocs, 'maplimits', 'absmax'); colorbar; title('Difference (Patients - Controls)');
nexttile(t_avg);
topoplot(sdi_diff_avg.*sig_mask_groupdiff, EEG.chanlocs, 'maplimits', 'absmax', 'style', 'map', 'emarker2', {find(sig_mask_groupdiff),'o','k',5,1}); colorbar; title(['Significant Differences (p < ' num2str(stat_alpha) ' maxT-corr.)']);

ax_main_title_avg = axes('Parent', fig_avg, 'Position', [0 0 1 1], 'Visible', 'off');
title_text_avg = 'Structural Decoupling Index (Alpha Power on Dynamic Trial FC Graphs)';
...
for g = 1:length(groups_to_analyze)
    current_group = groups_to_analyze{g};
    fig_corr = figure('Name', ['SDI-WASI Correlation - ' current_group.Name]);
    t_corr = tiledlayout(1, 2, 'Padding', 'compact');
    nexttile(t_corr);
    r_map = corr_results.(current_group.Name).r_real;
    topoplot(r_map, EEG.chanlocs, 'maplimits', 'absmax'); colorbar; title(['Correlation with WASI (r-values) - ' current_group.Name]);
    nexttile(t_corr);
    sig_mask_corr = corr_results.(current_group.Name).sig_mask;
    topoplot(r_map.*sig_mask_corr, EEG.chanlocs, 'maplimits', 'absmax', 'style', 'map', 'emarker2', {find(sig_mask_corr),'o','k',5,1}); colorbar; title(['Significant Correlations (p < ' num2str(stat_alpha) ' maxR-corr.)']);
    ax_main_title_corr = axes('Parent', fig_corr, 'Position', [0 0 1 1], 'Visible', 'off');
    title_text_corr = ['SDI Correlation with WASI - ' current_group.Name];
    text(0.5, 0.97, title_text_corr, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'top', 'FontSize', 14, 'FontWeight', 'bold', 'Parent', ax_main_title_corr);
end

% **NEW**: Plot the difference in correlations between groups
fig_corr_diff = figure('Name', 'Difference in SDI-WASI Correlation (Patients - Controls)');
t_corr_diff = tiledlayout(1, 2, 'Padding', 'compact');

nexttile(t_corr_diff);
diff_map = corr_results.Difference.diff_map_real;
topoplot(diff_map, EEG.chanlocs, 'maplimits', 'absmax'); colorbar; title('Difference Map (r_{patients} - r_{controls})');

nexttile(t_corr_diff);
sig_mask_diff = corr_results.Difference.sig_mask;
topoplot(diff_map.*sig_mask_diff, EEG.chanlocs, 'maplimits', 'absmax', 'style', 'map', 'emarker2', {find(sig_mask_diff),'o','k',5,1}); colorbar; title(['Significant Differences (p < ' num2str(stat_alpha) ' maxDiff-corr.)']);

ax_main_title_diff = axes('Parent', fig_corr_diff, 'Position', [0 0 1 1], 'Visible', 'off');
title_text_diff = 'Difference in SDI-WASI Correlation Between Groups';
text(0.5, 0.97, title_text_diff, 'HorizontalAlignment', 'center', 'VerticalAlignment', 'top', 'FontSize', 14, 'FontWeight', 'bold', 'Parent', ax_main_title_diff);
